...
`/api/icon/currency-exchange/{id}` (id `%2F`-escaped) out of the
`currency-exchange/` sub-directory of `ICON_CACHE_DIR`,
which must be pre-seeded on production — see [Icons](ICONS.md).
