...
`internal/icons`. Each name is resolved against a name→URL map and the image is
...
- **The map is compiled into the binary** (`//go:embed urls/*.json` in
  `internal/icons/icons.go`; one file per category since the 2026-09-04
...
(`internal/icons/icons.go:145`) returns the disk copy unconditionally when
...
`internal/icons` cache, constructed through the exported
`icons.NewWithMap(urls, cacheDir)` over a second embedded map —
`internal/exchange/itemdata/items.json`, generated by
`scripts/generate-currency-exchange-items.py` and read out as
`exchange.IconURLs()`. `icons.New` is now a thin wrapper that loads the gem
...
aliases; `icons` now holds no default directory at all, because a default
...
`internal/icons/gem-icon-urls.json` is replaced by
`internal/icons/urls/gems.json` (763 skill gems) and
`internal/icons/urls/items.json` (the two lab offerings, which are items the
...
**Decision 3 is implemented.** `internal/icons`'s cache filename is
