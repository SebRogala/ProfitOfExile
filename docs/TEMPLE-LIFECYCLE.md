...
**Amended 2026-09-11 (POE-275 WI-2, owner)**: the cold sweep runs OFF the loop thread, one at a
time, and the 650 ms placed recheck keeps running while it searches; a recheck that anchors
cancels it and its origin wins, and a sweep that found the panel is read only once a later capture
confirms the origin. A null or unplaced slice no longer spends its sweep on the first miss: it
sweeps every `NULL_SWEEP_EVERY` = 3 consecutive clean misses, up to `NULL_SWEEP_CAP` = 10 per
`(temple_epoch, temple_rearm)` key (both provisional). Every sweep writes one measured line. The
placed path is unchanged: a placed miss sweeps only under Re-arm, once per key. Row 3, the
"Placed-origin verification" residual, the new start-up-probe residual, "Cadences and budgets",
"Where each rule lives" and "Owner decisions" carry it; the "one cold-start fallback per key"
sentences tagged earlier are history.
Read this before
...
- **Placed-origin verification** checks the Entrance placement from the current
  ...
  remains residual/debug-only. **Amended 2026-09-11 (POE-275 WI-2, owner):** the
  null or unplaced slice no longer gets one fallback per key on its first miss —
  it sweeps every `NULL_SWEEP_EVERY` = 3 consecutive clean misses, up to
  `NULL_SWEEP_CAP` = 10 per key, and the withheld rule rides on top: the first
  found-but-withheld result lets the cadence go on, a second ends that key's null
  sweeps until the key changes (as does a found anchor that filled the slice).
  Every sweep runs off the loop; a found origin is confirmed on a later capture
  before it is read. What is left of this residual: a sheet on a never-anchored
  screen is found about 2 s of misses plus one sweep after the loop starts
  looking, not one sweep — the price of not spending the key's sweep before the
  sheet can be open.
...
- **A module switched on with the sheet already open stands down unswept when there is no
  screen slice, or a seed that misses the sheet** (POE-275 WI-2, 2026-09-11). The start-up probe
  opens the gate for exactly one tick, and on such a screen that tick is a first miss:
  `run::cold_sweep_reason` sweeps only on the third consecutive one, so the probe buys no sweep,
  finds nothing, and the next iteration stands the loop down. Accepted as the consequence of the
  owner's "do not spend the sweep on the first miss", which the rule applies to every arm source
...
- The placed-origin recheck runs at the existing **650 ms** detect cadence and does one
  ...
  also remembered after a successful read. **Amended 2026-09-11 (POE-275 WI-2, owner): the null
  slice now HAS a sweep cadence.** A null or unplaced slice starts a sweep on every
  `NULL_SWEEP_EVERY` = **3** consecutive clean misses (about 2 s of recheck at 650 ms), up to
  `NULL_SWEEP_CAP` = **10** sweeps per `(temple_epoch, temple_rearm)` key; the count restarts
  when a sweep starts and does not move while one is in flight, so the next sweep needs three
  misses after the last one ended. A held miss, a retire and a failed grab do not count; a
  sighting restarts the count; a key change resets the count and the cap. A placed recheck that
  anchors on a screen with a seeded but never-anchored origin ENDS the key's null sweeps
  (`SweepBudget::on_recheck`): the seed was right, so a sweep under that key could only find what
  the recheck found, and without the end the cadence restarted after every close in the same key
  (up to 10 sheet-less sweeps across a Temple of Atzoatl run). Both numbers are provisional, to be
  corrected from the per-sweep line below. The placed-miss sweep keeps its one per key under
  Re-arm.
- The cold fallback remains the pyramid sweep (5.3 s in the release container, POE-234),
  ...
  **Since 2026-09-11 (POE-275 WI-2) it runs off the loop thread** (`run::SweepSlot`), one at a
  time: the 650 ms detect cadence — the placed recheck with it — keeps running while it searches,
  where until then no tick ran for the sweep's whole duration (5.3 s release, ~30 s on the PC's
  debug build). A placed recheck that anchors cancels the sweep and its origin wins; a key change,
  a stand-down and the module stopping cancel it too, and the loop never waits for it to end. A
  sweep that found the panel is read only after a later capture re-finds it at the swept origin
  and scale (`run::confirm_swept`). Every sweep writes ONE line when it ends, `Temple: cold sweep
  (NullSlice | PlacedMiss[, attempt k of 10]) at WxH — N ms, found at (x,y) scale s — confirmed
  on the current frame | found no layout panel | cancelled by recheck | key change | stand-down |
  stop | …; release | debug build` (`run::sweep_line`), which replaces the `sweep found no layout
  panel` line. The release number on the PC has never been recorded; this line is how it will be.
  A measurement, never a switch.
...
- Cold fallback: a null or unplaced screen slice spends the pyramid sweep once per
  ...
  debug button. **Amended 2026-09-11 (POE-275 WI-2, owner):** the null or unplaced slice spends
  it every 3 consecutive clean misses up to 10 per key rather than once per key, the
  found-but-withheld rule rides on top of that cadence (a second one ends the key's null sweeps),
  and the sweep runs off the loop — see the bullets above.
...
| tick order: prune → hint from placements → placed-origin recheck → explicit fallback when qualified → full read → publish | `temple/run.rs` (`tick`, `cheap_hint_from_screen`, `cold_sweep_reason`, `cold_sweep`, `full_read`) — current POE-269 path. **Amended 2026-09-11 (POE-275 WI-2):** a sweep in flight is cancelled on a key change, then after the recheck the sweep slot is settled — a recheck that anchored wins and cancels it, a found origin is used only as re-found on this capture — and only a clean miss with nothing live may START a sweep, which runs off the loop: prune → hint → recheck → settle the sweep in flight → sighting: full read → publish, or miss: maybe start a sweep. Homes: `SweepSlot` (`launch`, `settle`, `keep_only`, `cancel`), `confirm_swept`, `SweepBudget` with `NULL_SWEEP_EVERY` = 3 and `NULL_SWEEP_CAP` = 10 (charged by `cold_sweep_reason`), `sweep_line`; the stand-down and module-stop cancels are in `run_loop` |
...
2026-09-11 (POE-275 WI-2), relayed verbatim: *"(1) Requirements 2-4 as written: the sweep runs
off the loop thread (or in bounded slices), the 650 ms placed recheck keeps running during it, a
recheck that lands cancels it and its origin wins; log duration + build once per sweep. (2)
Requirement 1 is closed for the PLACED path by 05a51a8 - keep that, no placed-miss sweep under
AlvaStart/TempleArea. It is NOT closed for a null/unplaced slice: cold_sweep_reason still spends
the NullSlice sweep on the first cheap miss after the start line, before the sheet can be open,
and with no hint detect_cheap returns Nothing for the rest of that key, so the player needs
Re-arm/Recalibrate (the owner's friend hit exactly this on a first run). Fix: on a null/unplaced
slice do not spend the sweep on the first miss; retry it off the loop every N consecutive misses
while armed; choose N and any per-key cap in the plan and state them. No END-line rule: since
WI-1 a non-start Alva line stands the capture down."* Encoded in `run.rs`: the sweep runs on its
own thread over its own frame (`SweepSlot`), the loop keeps ticking and polls it, a placed
recheck that anchors cancels it and wins, and a found origin is read only after a later capture
confirms it (`confirm_swept`) — the sheet can close during a multi-second search. The null slice
sweeps every `NULL_SWEEP_EVERY` = 3 consecutive clean misses up to `NULL_SWEEP_CAP` = 10 per key
(`cold_sweep_reason`, `SweepBudget`), both provisional and to be corrected from the one
`Temple: cold sweep …` line each sweep writes (`sweep_line`, which carries the build). The
placed path is unchanged. The cost is the start-up-probe residual above: a module switched on over
an open sheet with no screen slice, or on a seed that misses the sheet, stands down unswept, and
Re-arm is its answer.
