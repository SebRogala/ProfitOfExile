...
premise for module placement. The per-tick anchor recheck, cold-sweep fallback and
`GEOMETRY_READS_CAP` prose below describe shipped behavior **current until POE-269**; the
placed-origin verification and one-read fallback belong to ADR-024.
Read this before
...
- **Current until POE-269: more than `GEOMETRY_READS_CAP` = 8 geometry changes
  inside one incursion**, with no Alva line and no room change between them,
  leave the overlay drawing the ROIs of the last read — a few px stale — until
  Re-arm or the next room. This is the retired `window drags` fallback described
  by the current loop; ADR-024 replaces it with placed-origin verification and
  one fallback locate. The alternative was 28 OCR calls every 650 ms for an
