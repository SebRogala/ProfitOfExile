...
- **Placed-origin verification** checks the Entrance placement from the current
  ...
  key only for an `ArmSource::Trigger(_)` arm (AlvaStart, TempleArea or Manual);
...
- The placed-origin recheck runs at the existing **650 ms** detect cadence and does one
  ...
  and a below-floor placed miss gets one fallback per the same key for any
  `ArmSource::Trigger(_)` arm (AlvaStart, TempleArea or Manual). A sweep contradiction logs the
...
- Cold fallback: a null or unplaced screen slice spends the pyramid sweep once per
  ...
  the same key for any `ArmSource::Trigger(_)` arm. A fallback that finds an anchor
