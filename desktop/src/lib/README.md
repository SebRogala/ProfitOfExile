...
| File | Exports | Description |
|------|---------|-------------|
| `exchange/view.ts` | `MODE_OPTIONS`, `parseMode()`, `deriveState()`, `formatTimeAgo()`, `formatTime()`, `formatEdge()`, `formatVolume()`, `formatLegPrice()`, `legLabel()`, `iconSrc()`, `refetchDelay()`, `REFETCH_DEBOUNCE_MS`, `REFETCH_JITTER_MS`, `ViewState`, `ViewStateKind`, `ViewStateInput`, `CurrencyExchangeMode` (re-export) | Presentation derivations for `CurrencyExchangePage`, the sibling of `temple/view.ts` and `mercenaries/ladder-view.ts` and for the same reason: a `.svelte` file has no unit-test harness here. Decides nothing about the market — the server (POE-175) owns the ranking, the edge and the depth; this only words them. `deriveState()` is the single owner of the header's five states, including the rule that an error outranks `warming`; `refetchDelay()` owns the Mercure debounce + jitter, the same two values `LabPage.svelte` runs against the same publish. `iconSrc()` (POE-177) joins the API base onto a leg's API-relative `itemIcon`/`quoteIcon` path — the server serves that artwork from its own cache because production cannot reach poewiki (ADR-012) — and answers `null` for both `null` and `""`, since an empty path joined onto the base requests the base itself. Tests live in `exchange/view.test.ts`. |
...
| `pages/CurrencyExchangePage.svelte` | Currency Exchange (POE-176) — the server's ranked arbitrage plays as one table (rank, direct/1-hop, the legs of the swap in order, edge, per-hour depth, hours seen) under a persisted `currencyExchangeMode` filter (`all`/`direct`/`1-hop`, ADR-013). The header's state comes from `$lib/exchange/view`'s `deriveState`, re-derived against a `now` that ticks every 30 s. Each leg chip carries the ITEM's icon only (`ItemIcon` + `iconSrc`, POE-177) — one icon per chip keeps the rows compact, so the quote currency's icon is deliberately not rendered. A failed fetch keeps the last plays on screen and says how stale they are — it never blanks the table. Opens no Mercure connection: `LabPage` re-emits `poe/currency-exchange/updated` as the Tauri event `currency-exchange-updated`, which this page answers with a jittered debounced refetch. |
