...
export const EXCHANGE_TOOLTIPS: Record<string, string> = {
	ROI: 'Absolute profit in chaos orbs across the WHOLE worthwhile run — the Scale column’s flip count, the same run the route above is priced for — if BOTH of the hour’s extreme prices had been there for you on every exchange, net of one price step of undercut on each of the play’s trades. That is the hour’s best case, not what the play pays: Exp. ROI beside it is the measured answer, and it is what the Scale column and the ranking use. The route ends on the same figure: its LAST STEP totals what the hour’s best case would have paid, which is the Spend plus this column — except where a step has no run total to print at all, which its own hover says. A play with no positive expectation has no run to repeat toward, so its money columns fall back to what ONE exchange gains — the same SIZE the route’s two ends fall back to, so the row is never half one size and half the other. The size is all they share: the route’s Get end is the Spend plus Exp. ROI on every row, never the Spend plus this column, which is why a measured loser can end below what it cost. Both identities are in chaos, and a divine-entry route prints them at the divine rate. Every figure here is the last settled hour, so verify the route in game before committing.',
	'ROI%':
		'Return on investment as a percentage — ROI divided by what it cost. Scale-free in the literal sense: the run multiplies the ROI and Investment columns by the same flip count, so this is the same percentage per exchange as it is per run, and it compares plays across price tiers either way. NET is net of one price step of undercut on every one of the play’s trades, the best case an order that actually gets taken could have had in that hour. RAW is the same round trip at the hour’s raw extremes — never below NET, and the gap between them is what those steps cost. Neither is what the table is ranked by any more: Exp. ROI is. NET is still what your Gates row judges.',
	'Exp. ROI':
		'What posting this play’s orders would have paid, in chaos across the WHOLE worthwhile run — the same chaos the route’s “keep ≈” line carries, or its “lose ≈” line when the measurement came out negative, and the number the Scale column repeats the play toward. The route’s Get end is the Spend plus this figure in BOTH cases, the negative one included, so a play that measured a loss ends the row below what it cost. That identity is in chaos, and a divine-entry route prints it at the divine rate. Every hour of the last day is replayed: your buy goes up one step above the hour’s cheapest buy price and chases the market up if it does not fill, your sell sits one step under the dearest sell price and waits up to three hours for someone to take it, and whatever never sold is dumped at the last hour of that wait, halfway between its average price and one step under its dearest. The mean of those outcomes is what ONE exchange pays, and this figure is that mean across the run — so on a play that has a run it always clears about 100 chaos, and it is the run’s SIZE, in the Scale column, that separates a strong play from a weak one. The table is still ranked on the per-exchange mean, which is why the Exp. ROI sort is the order the server sent and not a re-sort of this column. It CAN be negative, and then it is one exchange: a play the simulation expects to lose has no run to repeat toward, so the figure falls back to the per-exchange mean. The play is still shown and simply ranks below the ones that measured well. n is how many hours were replayed; LOW COVERAGE means too few of them to trust the mean, not that the play is bad. Measured across 960 top-20 play-hours the ROI column overstates this by four to eight times, and that measurement is of DIRECT flips: a 1-hop route is replayed the same way but nothing has checked that a triangle behaves like a flip.',
	Investment:
		'Chaos you need liquid for the WHOLE worthwhile run — the cost at the undercut entry, times the Scale column’s flip count. It is the exact figure the Scale column’s “N c in” sub-line prints and the exact figure the filter bar’s Run cost bounds are compared against, so a bankroll ceiling there and this column can never mean two different trips. A play with no positive expectation has no run, and then this is what ONE exchange costs — the same fallback every money figure on that row takes. Switch those bounds to divine for the large ones — converted at the divine/chaos rate from the same feed hour.',
	...
	Route:
		'The whole worthwhile RUN as five slots: what you spend, the two or three trades, what you get back. The size is the Scale column’s — repeat the play until it is expected to clear about 100 chaos — so step 1 says how many to buy and every amount is that many. Spend is what those buys cost at the undercut entry, in the currency you actually pay with: a route entered in divine reads in divine, with the chaos equivalent under it. Get is that same figure back plus the run’s expected profit, and the line under it says the profit in chaos. Each step names the ITEM it acts on and reads as a RUN TOTAL carrying a ≈ — “buy 12 for ≈ 424c”, “sell 12 for ≈ 2.97 div” — what the whole run costs or brings in, never a per-unit price like “0.25 div each”, which the exchange has no field for and the game will not let you post. The currency is on every step — c for chaos, div for divine — so a route that trades against divine shows you where; a step quoted in neither prints its amount bare rather than label it with a unit word that would be wrong. EVERY amount on the row, the steps included, is priced at the order that actually fills, one price step tighter than the hour’s posted extreme; hover a step to see what that market printed. The step always shows the whole run, and where a market only posts in lots the run does not divide by, that hover says how many whole orders that is and what is left over. All three of those — the run total, the fill price, no per-unit rate — hold except where a step has no run total to print at all: such a line falls back to what its own market posts — one order of it, or the bare per-unit rate when that market posted no pair — and its own hover says so. The last step of a 1-hop converts the proceeds of the sale into the end of the chain. That last total is what the hour’s best case would have paid — the Spend plus the ROI column — while Get is what the last day measured, the Spend plus the Exp. ROI column; the gap between the two is the gap between those two columns. Both identities are in chaos, and a divine-entry route prints them at the divine rate. A play with no positive expectation has no run size, so its steps count ONE exchange, its two ends stay in the currency you pay with, and Get is still the Spend plus the measured expectation — on such a row a loss, or exactly nothing.',
	Trend:
	...
	Scale:
		'The flip count at which this play clears about 100 chaos of EXPECTED gain, what those flips tie up, and how long the market needs to absorb them. It counts Exp. ROI and not ROI, so the size is the one you would actually have had to run: a play whose expectation is 3c an exchange reads ×34, one worth 150c reads ×1, and a play that is not expected to gain at all shows a dash — there is no number of repeats that turns a loss into 100c. The hours are UNCONTESTED — they assume the WHOLE hourly volume on the play’s thinnest trade is yours and nobody else is competing for it — so read the wait as the floor on the time, not the time. Green is inside the hour, which is the only case where the ROI was computed against a book that will still be there.',
	Hours:
	...
	'Run cost':
		'What the worthwhile run ties up — the same figure the Investment column and the Scale column’s “N c in” sub-line print, not the cost of one exchange. Not two figures that happen to agree: the bound reads the very number the column prints, so a ceiling you type here can never let through a row whose Investment sits above it. Set the min/max to see only what your bankroll covers; the divine toggle converts the bounds at the newest hour’s rate. A play with no run is judged on what one exchange costs, which is what its Investment column shows too.',
	Gates:
	...
	'Min item price':
		'The least ONE exchange may COST to enter, in chaos — the price of the thing you are flipping. It is deliberately per-exchange and the Investment column beside it is not: that column counts the whole worthwhile run, so nothing on screen prints the number this floor is compared against. That stays deliberate because the 0.5 is a calibrated price of a THING: judging it against a run would multiply the level you type by each row’s own flip count, so one number in the box would mean a different floor on every row and stop being a level at all. This is the only filter this app arms for you: it ships at 0.5, which drops the bottom of the sub-chaos tier (Clear Oil and its neighbours). Not because those markets are fake — they are as real and as finely priced as any other — but because an entry under half a chaos pays too little per flip to be worth the repeats, so those rows crowd out the ones you would act on. The 0.5 is chosen to keep the fragment and oil tier: a flip entered at 0.5c or more is still on the table, and only the very bottom goes. Everything else here starts off, and the server sends everything it can stand behind. To disable it, type 0 — the table then shows the fractional stuff too. Blanking the box does NOT disable it — an empty box means whatever this build ships, which is the 0.5. Exactly 0.5c gets through; only cheaper is dropped. A 1-hop route is judged on the same number, the chaos it costs to enter.',
	'Min profit':
		'The least chaos a play must gain on ONE exchange, at the hour’s best-case prices — the best-case number the ROI column is built from, before that column multiplies it up to the whole run, and never Exp. ROI. A gate asks whether the market is worth trading at all, never how far the play has to be repeated to pay, which is the Scale column’s answer; so like Min item price it reads one exchange while the columns read the run, and no column prints the figure it compares against. That is kept on purpose: the 3 below is the floor the SERVER applied per exchange, and re-pointing this gate at the run would multiply it by each row’s own flip count — “3c profit” would mean 3c per run on a ×1 row and thirty-four times that on a ×34 row, so the number in the box would stop being a level at all. Empty (or 0) is off, which is how the cheap plays stay visible: sacrifice fragments and the like gain a fraction of a chaos each and only add up on volume. Type 3 for the old server floor.',
	'Min turnover':
