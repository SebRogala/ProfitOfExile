...
export const EXCHANGE_TOOLTIPS: Record<string, string> = {
	ROI: 'Absolute profit in chaos orbs at the size the row reads at — ONE POSTING of the market you enter on when that market prices in chaos, the whole worthwhile run when it prices in divine (the Scale column’s flip count). Either way it is the size the route above is priced for, and it is what you would have made if BOTH of the hour’s extreme prices had been there for you on every exchange, net of one price step of undercut on each of the play’s trades. That is the hour’s best case, not what the play pays: Exp. ROI beside it is the measured answer, and it is what the Scale column and the ranking use. The route ends on the same figure: its LAST STEP totals what the hour’s best case would have paid, which is the Spend plus this column — except where a step has no total to print at all, which its own hover says. A play with no positive expectation and no chaos posting to read counts ONE exchange, the same SIZE the route’s two ends fall back to, so the row is never half one size and half the other. The size is all they share: the route’s Get end is the Spend plus Exp. ROI on every row, never the Spend plus this column, which is why a measured loser can end below what it cost. Both identities are in chaos, and a divine-entry route prints them at the divine rate. Every figure here is the last settled hour, so verify the route in game before committing.',
	'ROI%':
