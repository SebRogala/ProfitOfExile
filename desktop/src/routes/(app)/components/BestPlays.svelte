...
<table class="plays-table">
	<thead>
		<tr>
			<th class="col-name"><Tooltip text="Transfigured gem name">Gem</Tooltip></th>
			{#if showVariantColumn}<th class="col-var"><Tooltip text="Gem variant: level/quality (e.g. 20/20 = level 20, 20% quality)">Var</Tooltip></th>{/if}
			<th class="col-tier"><Tooltip text="<b>Price Tier</b> — per variant, dynamic boundaries.<br><br><span style='color:#fbbf24'>TOP</span> — Monopoly outliers (gap-detected)<br><span style='color:#fb923c'>HIGH</span> — Within 30% of top gem<br><span style='color:#c084fc'>MID-HIGH</span> — Above 50% of HIGH boundary<br><span style='color:#94a3b8'>MID</span> — Natural gap above LOW<br><span style='color:#64748b'>LOW</span> — Marginal profit above FLOOR<br><span style='color:#475569'>FLOOR</span> — Below 8% of top-5 average<br><br>Low-confidence gems (thin market) excluded from tier computation. Boundaries adapt to market prices automatically.">Tier</Tooltip></th>
			<th class="col-num"><Tooltip text="Current ninja price of the transfigured gem">Price</Tooltip></th>
			<th class="col-num"><Tooltip text={sortBy === 'riskAdjusted' ? 'Risk-adjusted ROI: raw ROI weighted by sellability and market stability' : METRIC_TOOLTIPS.ROI}>{sortBy === 'riskAdjusted' ? 'Adj ROI' : 'ROI'}</Tooltip></th>
	...
	<tbody>
		{#each sorted as gem, i}
			...
			>
				...
				<td class="col-sell">
					<Tooltip text="{gem.sellabilityLabel} ({gem.sellability})"><span class="sell-score sell-{gem.sellabilityLabel.toLowerCase().replace(' ', '-')}">{gem.sellability}</span></Tooltip>
				</td>
