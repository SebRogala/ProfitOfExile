...
impl SeedBlocklist {
    ...
    ///
    /// A list written by an EARLIER build keys families the way that build
    /// derived them, so every entry is folded through
    /// [`super::vocab::alias_family`] on read (POE-211). Without it, a family
    /// the user threw the seed out of comes back the moment its name is
    /// aliased — the entry would block a key nothing derives any more, and the
    /// re-derivation would install the same wrong art under the new one.
    pub fn load(dir: &Path) -> Self {
        let Ok(raw) = std::fs::read_to_string(dir.join(BLOCKLIST_FILE)) else {
            return Self::default();
        };
        let mut out: Self = serde_json::from_str(&raw).unwrap_or_default();
        for family in &mut out.families {
            let aliased = super::vocab::alias_family(family).to_string();
            *family = aliased;
        }
        // The fold can COLLIDE: a list that blocked both spellings — one seed
        // thrown out before the upgrade, one after — re-keys to the same name
        // twice, and the next `block_family` would `save` that duplicate back
        // to disk, forever. `blocks` would still answer correctly; the file
        // would just grow a copy per fold.
        out.families.sort();
        out.families.dedup();
        out
    }
...
mod tests {
    ...
    /// Families whose gem is named explicitly, because neither name rule finds
    /// it (POE-211).
    ///
    ...
    /// a rule that guessed at aliased spellings would map families nobody
    /// checked.
    const SEED_GEM_OVERRIDES: [(&str, &str); 1] =
        [("Area of Effect", "Increased Area of Effect Support")];
    ...
    /// spec + POE-211).
    ///
    /// [`SEED_GEM_OVERRIDES`] is tried FIRST and returns: an override is a
    /// statement about one family, not a fallback for when the rules miss, and
    /// trying it first is also what keeps the counts test from double-counting
    /// a family that would satisfy a name rule too.
    ///
    ...
    fn mapped_gem(family: &str, keys: &std::collections::BTreeSet<String>) -> Option<String> {
        if let Some((_, gem)) = SEED_GEM_OVERRIDES.iter().find(|(f, _)| *f == family) {
            return Some((*gem).to_string());
        }
    ...
    fn header(excluded: &[String]) -> Vec<String> {
        let mut lines: Vec<String> = [
            ...
            "family/gem: an explicit override first (1: Area of Effect -> Increased Area of Effect",
    ...
    /// The two name rules produce 46 + 4 hits, the override adds 1, and every
    /// one of the seven hand-graded families is among them.
    ///
    ...
    /// a map that quietly seeds fewer families. The override is counted
    /// SEPARATELY and its families are skipped by the two rules here, which is
    /// what makes the sum an accounting of the map rather than a tautology: a
    /// row reachable both ways would otherwise be counted twice and the map
    /// would look one row short.
    #[test]
    fn the_rules_hit_forty_six_plus_four_families_and_the_override_one() {
        let keys = gem_icon_keys();
        let mut exact = 0usize;
        let mut damage = 0usize;
        for (family, _) in families() {
            if SEED_GEM_OVERRIDES.iter().any(|(f, _)| *f == family) {
                continue;
            }
        ...
        assert_eq!((exact, damage), (46, 4));
        let map = load_map().expect("the map parses");
        assert_eq!(map.len(), exact + damage + SEED_GEM_OVERRIDES.len());
        for (family, gem) in SEED_GEM_OVERRIDES {
            let row = map
                .iter()
                .find(|e| e.family == family)
                .unwrap_or_else(|| panic!("{family} is overridden but absent from the map"));
            assert_eq!(row.gem, gem, "{family} did not take its override");
        }
    ...
    /// An override BEATS the name rules, rather than covering for them when
    /// they miss.
    ///
    ...
    fn an_override_beats_the_name_rules() {
        let keys: std::collections::BTreeSet<String> = [
            "Area of Effect Support".to_string(),
            "Area of Effect Damage Support".to_string(),
            "Increased Area of Effect Support".to_string(),
        ]
        .into_iter()
        .collect();

        assert_eq!(
            mapped_gem("Area of Effect", &keys).as_deref(),
            Some("Increased Area of Effect Support"),
        );
        assert_eq!(
            mapped_gem("Chain", &keys),
            None,
            "a family the table does not carry still goes through the name rules",
        );
    }
    ...
    /// A blocklist written before POE-211 keys the family the way that build
    /// derived it, so `load` re-keys it through the alias.
    ///
    /// The ✕ is a statement about the ART, not about a spelling: without the
    /// re-key the entry would block a family nothing derives any more, and the
    /// next start would install the same rejected art under the folded name.
    #[test]
    fn the_blocklist_re_keys_a_family_the_alias_folded() {
        let dir = tmp_dir("block-aliased");
        std::fs::write(
            dir.join(BLOCKLIST_FILE),
            r#"{"families":["Increased Area of Effect","Chain"]}"#,
        )
        .expect("write");

        let list = SeedBlocklist::load(&dir);

        assert!(list.blocks("Area of Effect"), "families were {:?}", list.families);
        assert!(!list.blocks("Increased Area of Effect"));
        assert!(list.blocks("Chain"), "an unaliased entry must be left alone");
    }

    /// A list that blocked BOTH spellings — one seed thrown out before the
    /// upgrade, one after — folds to a single entry.
    ///
    /// Without the dedupe `blocks` still answers correctly, but the next
    /// `block_family` saves the duplicate back to disk and it never leaves.
    #[test]
    fn the_blocklist_folds_both_spellings_onto_one_entry() {
        let dir = tmp_dir("block-aliased-dupe");
        std::fs::write(
            dir.join(BLOCKLIST_FILE),
            r#"{"families":["Increased Area of Effect","Area of Effect"]}"#,
        )
        .expect("write");

        let list = SeedBlocklist::load(&dir);

        assert_eq!(list.families, ["Area of Effect".to_string()]);
    }
