...
//!
//! [`set_module_enabled`] is the ONE writer, and it always persists: a module
//! flag is the user's own choice and nothing else forces it. The widget-config
//! flow used to (POE-226) — it turned a module on to have a window to arrange
//! widgets in — and POE-241 removed that: arranging a widget raises the overlay
//! WINDOW's desired state in `routes/(app)/+layout.svelte` and starts no module
//! work at all, so there is no session force left to project around.
//!
