...
pub struct OfferView {
    ...
    /// The drop facts for the line this offer builds.
    #[serde(default)]
...
#[serde(rename_all = "camelCase")]
pub struct LineView {
    /// The line's box shape, serialized as `"chest"` or `"content"`. A payload
    /// from before WI-6 decodes as the chest-shaped box it drew.
    #[serde(default)]
    pub kind: LineKind,
    /// The draft prose for a content line, or `null` for a chest line.
    #[serde(default)]
    pub content: Option<String>,
    /// The architect the mod group is known by, or `null` where the line has
    /// no signature rare.
    ...
    pub quantity_pct: Option<[f64; 3]>,
    /// Area rarity bonus, same shape and same rule.
    pub rarity_pct: Option<[f64; 3]>,
}
...
fn offer_view(
    ...
    OfferView {
        ...
        recipe: resolved
            ...
            .map(recipe_view),
        line: resolved.as_ref().map(|r| line_view(r.line)),
    }
}

fn ladder_values<F>(tiers: &[drops::TierDrops; 3], select: F) -> Option<[f64; 3]>
where
    F: Fn(drops::TierDrops) -> Option<drops::Estimate>,
{
    ...
    Some(values.map(|value| value.expect("ladder values are complete").value()))
}

fn line_view(line: &rooms::RoomLine) -> LineView {
    let row = line.drops();
    let temple_mod = row.temple_mod();
    LineView {
        kind: row.kind(),
        content: row.content().map(str::to_string),
        mod_architect: temple_mod.map(|value| value.architect().to_string()),
        mod_hint: temple_mod.map(|value| value.item_hint().to_string()),
        mod_slots: temple_mod
            .map(|value| {
                value
                    .slots()
                    .iter()
...
mod tests {
    ...
    #[test]
    fn a_resolved_offer_carries_the_line_bonus_ladders() {
        let input = offer("Quipolatl", "Chamber of Iron", OfferKind::Upgrade);
        let view = offer_view(0, &input, None, &valued());
        let line = view.line.expect("a resolved offer carries line facts");

        assert_eq!(line.quantity_pct, Some([2.0, 4.0, 6.0]));
        assert_eq!(line.rarity_pct, Some([4.0, 8.0, 12.0]));
    }

    #[test]
    fn a_line_with_no_stated_bonus_publishes_none() {
        let input = offer("Quipolatl", "Museum of Artefacts", OfferKind::Upgrade);
        let view = offer_view(0, &input, None, &valued());
        let line = view.line.expect("a resolved offer carries line facts");

        assert_eq!(line.quantity_pct, None);
        assert_eq!(line.rarity_pct, None);
    }
    ...
    #[test]
    fn a_line_without_a_temple_mod_publishes_empty_mod_facts() {
        let input = offer("Quipolatl", "Chamber of Iron", OfferKind::Upgrade);
        let view = offer_view(0, &input, None, &valued());
        let line = view.line.expect("a resolved offer carries line facts");
    ...
    /// A line with no tier-3 chest publishes the prose the box draws in place
    /// of the chest row. Throne of Atziri is the case that also carries a vial,
    /// so the content cell has to reach the wire beside a drop rather than
    /// instead of one.
    #[test]
    fn a_content_line_publishes_its_kind_and_the_rooms_own_prose() {
        let input = offer("Quipolatl", "Throne of Atziri", OfferKind::Upgrade);
        let view = offer_view(0, &input, None, &valued());
        let line = view.line.expect("a resolved offer carries line facts");

        assert_eq!(line.kind, LineKind::Content);
        assert_eq!(line.content.as_deref(), Some("Queen Atziri"));
    }

    /// The other half, and the pair is the point: a `kind` wired to a constant
    /// would pass the content test alone, and a box reading `Chest` here draws
    /// `A + B -> C` for a room whose chest does not exist.
    #[test]
    fn a_chest_line_publishes_no_content_prose() {
        let input = offer("Puhuarte", "Crucible of Flame", OfferKind::Upgrade);
        let view = offer_view(0, &input, None, &valued());
        let line = view.line.expect("a resolved offer carries line facts");

        assert_eq!(line.kind, LineKind::Chest);
        assert_eq!(line.content, None);
    }

    #[test]
    fn an_unresolved_offer_publishes_no_line_facts() {
        let input = offer("Quipolatl", "Qwertz Chamber", OfferKind::Upgrade);
        let view = offer_view(0, &input, None, &valued());

        assert_eq!(view.line, None);
    }

    #[test]
    fn an_offer_json_without_line_decodes_as_no_line() {
        let view: OfferView = serde_json::from_str(
            r#"{"index":0,"architectName":"Guatelitzi","kind":"upgrade","printedTarget":"Sadist's Den","displayName":null,"builtTier":null,"rect":null}"#,
        )
        .expect("an older offer without line facts still decodes");

        assert_eq!(view.line, None);
    }

    /// A payload written before WI-6 carries line facts but neither of the two
    /// fields it added. It decodes as the chest-shaped box it drew: that is what
    /// `Default` for [`LineKind`] is for, and a `content` defaulting to anything
    /// but `None` would put prose on a box whose chest row is already drawn.
    #[test]
    fn a_line_json_without_kind_or_content_decodes_as_a_chest_line() {
        let line: LineView = serde_json::from_str(
            r#"{"modArchitect":null,"modHint":null,"modSlots":[],"modWorth":null,"quantityPct":null,"rarityPct":null}"#,
        )
        .expect("an older line without the WI-6 fields still decodes");
    ...
    fn a_populated_slice_json_is_pinned_for_the_typescript_mirror() {
            ...
            panel: Some(PanelView {
                ...
                offers: vec![OfferView {
                    ...
                    line: Some(LineView {
                        ...
                        content: Some("Extra monsters (dangerous)".to_string()),
                        mod_architect: None,
                        mod_hint: None,
                        mod_slots: Vec::new(),
                        mod_worth: None,
        ...
        let decoded: TempleSlice =
            serde_json::from_str(SAMPLE_SLICE_JSON).expect("sample slice parses back");
        assert_eq!(decoded, s, "the populated sample survives a JSON round-trip");
    }
    ...
    fn an_offer_publishes_the_same_chaos_the_advisor_ranked_on() {
        ...
        // Every term Locus names prices: its vial (WI-6 — the line has no chest
        // unique, so nothing is left unpriced).
        assert_eq!(value.priced, "market");
    ...
    ///
    /// Locus of Corruption has no chest unique: its altar makes Shadowstitch
    /// from the Sacrificial Garb the player brings, while the vial its architect
    /// rolls for upgrades somebody else's item.
    ...
    #[test]
    fn an_offer_whose_vial_upgrades_somebody_elses_unique_carries_no_recipe() {
        let layout = layout(Some(Slot::B0), &[], &[]);
        let rooms = board_rooms(&[(Slot::B0, "Catalyst of Corruption")]);
        let panel = panel(
            "Catalyst of Corruption",
