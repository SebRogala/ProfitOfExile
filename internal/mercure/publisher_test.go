...
func TestSignMercureJWT_producesValidHS256Token(t *testing.T) {
	...
	mercureClaim, ok := payload["mercure"].(map[string]any)
	if !ok {
		t.Fatal("payload missing mercure claim")
	}
	publish, ok := mercureClaim["publish"].([]any)
	if !ok || len(publish) == 0 {
...
func TestHubPublisher_Publish(t *testing.T) {
	var receivedTopic string

	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		body, _ := io.ReadAll(r.Body)
		// Parse form to extract topic.
		for _, part := range strings.Split(string(body), "&") {
			if strings.HasPrefix(part, "topic=") {
				receivedTopic = strings.TrimPrefix(part, "topic=")
			}
		}
		w.WriteHeader(http.StatusOK)
	}))
	...
	pub := &HubPublisher{URL: server.URL, Secret: "test-secret"}
	err := pub.Publish(context.Background(), "poe/test/topic", `{"hello":"world"}`)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	if receivedTopic != "poe%2Ftest%2Ftopic" && receivedTopic != "poe/test/topic" {
		t.Errorf("topic = %q, want poe/test/topic", receivedTopic)
	}
}
